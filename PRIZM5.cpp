...
void PRIZM5::setMotorSpeedPID(int P, int I, int D){	//=== Change the speed PID parameters for DC Chip
...
void PRIZM5::setMotorTargetPID(int P, int I, int D){	//=== Change the target PID parameters for DC chip 
...
int PRIZM5::readDCFirmware(){	  //==== Request firmware version of DC motor chip
...
int PRIZM5::readSVOFirmware(){		//==== Request firmware version of Servo motor chip
...
void PRIZM5::PrizmBegin(){  //======= Send a SW reset to all EXPANSIONansion port I2C 								
  //Wire.begin();
Serial.println("Waiting for start button");
  ...
  while(readStartButton()==1){};   		// wait for the program start (green) button pressed
...
void PRIZM5::PrizmEnd(){  //======= Send a SW reset to all I2C devices(resets everything) This is done mainly to stop all motors
...
int PRIZM5::readBatteryVoltage(){
 ...
 // Bvoltage = (Bvoltage*256)+byte2;
  delay(10);
   float Bvoltage=(float)Bvolt;
   //Bvoltage=Bvoltage/10.0;
   return Bvoltage;
}
...
int PRIZM5::readLineSensor(int pin){     // Can sense black or white (follow the edge of a black line on a white surface)
...
int PRIZM5::readSonicSensorCM(int pin){   // Returns distance of object from sensor in Centimeters
/*
...
int PRIZM5::readSonicSensorIN(int pin){   // Returns distance of object from sensor in Inches
  Wire.beginTransmission(9);
  Wire.write(0x3C); 
    Wire.endTransmission(); 
  delay(10);
...
void PRIZM5::setGreenLED (int state){
  Wire.beginTransmission(9);
  Wire.write(7);  
  Wire.write(state);                  	
  Wire.endTransmission();  
...
void PRIZM5::setRedLED (int state){
  Wire.beginTransmission(9);
  Wire.write(6);  	
  Wire.write(state);                  	
  Wire.endTransmission(); 
...
int PRIZM5::readStartButton(){       //============== function returns; unpressed button == 0; pressed button == 1
...
int PRIZM5::readDigitalPin(int pin){       //============== function returns; unpressed button == 0; pressed button == 1
  ...
  delay(10);
    Wire.requestFrom(9, 1); 
     byte pinBack  = Wire.read(); 
     return pinBack;       	

}

int PRIZM5::readAnalogPin(int pin){       //============== function returns; unpressed button == 0; pressed button == 1
  pin=pin+40;
  Wire.beginTransmission(9);
  Wire.write(pin); 
    Wire.endTransmission(); 
  delay(10);
    ...
    int pinBack=(highbyte<<8)+lowbyte; 
     return pinBack;       	

}


void PRIZM5::setServoSpeed (int channel, int servospeed){   //=========== function for setting PRIZM servo speeds individually
...
void PRIZM5::setServoSpeeds (int servospeed1, int servospeed2, int servospeed3, int servospeed4, int servospeed5, int servospeed6){   // function to set all PRIZM servo speeds at once
...
void PRIZM5::setServoPosition (int channel, int servoposition){   //function to set PRIZM servo positions individually
...
void PRIZM5::setServoPositions (int servoposition1,int servoposition2,int servoposition3,int servoposition4,int servoposition5,int servoposition6){  // Sets all PRIZM servo positions at once
...
void PRIZM5::setCRServoState (int channel, int servospeed){   // function to set PRIZM CR servos speed and direction -100|0|100
...
int PRIZM5::readServoPosition (int channel){		// read the servo PRIZM servo positions
...
void PRIZM5::setMotorPower(int channel, int power)	// set Motor Channel power on PRIZM
{
...
void PRIZM5::setMotorPowers (int power1, int power2){     //power only Block Command for PRIZM Motor 1 and 2 (both in one transmission)
...
void PRIZM5::setMotorSpeed (int channel, long Mspeed){      // === set speed of each PRIZM DC motor == requires a 1440 CPR installed encoder to do the PID
...
void PRIZM5::setMotorSpeeds (long Mspeed1, long Mspeed2){      // === BLOCK write to set speeds of both PRIZM motors at once == 1440 CPR encoder must be installed to do PID
...
void PRIZM5::setMotorTarget (int channel, long Mspeed, long Mtarget){      // === set speed and encoder target of each PRIZM DC motor == requires a 1440 CPR encoder to do the PID
...
void PRIZM5::setMotorTargets (long Mspeed1, long Mtarget1, long Mspeed2, long Mtarget2){      // === BLOCK WRITE set speed and encoder target of both PRIZM DC motors == requires a 1440 CPR encoder to do the PID
...
void PRIZM5::setMotorDegree (int channel, long Mspeed, long Mdegrees){      // === set speed and encoder target of each PRIZM DC motor in DEGREES  == requires a 1440 CPR encoder to do the PID
...
void PRIZM5::setMotorDegrees (long Mspeed1, long Mdegrees1, long Mspeed2, long Mdegrees2){      // === BLOCK WRITE set speed and encoder target in DEGREES of both PRIZM DC motors == requires a 1440 CPR encoder to do the PID
...
long PRIZM5::readEncoderCount (int channel){   // ============================= READ PRIZM ENCODER DATA COUNTS ====================================
...
long PRIZM5::readEncoderDegrees (int channel){   // ============================= READ PRIZM ENCODER DATA DEGREES ====================================
...
void PRIZM5::resetEncoder (int channel){    // =================== RESET PRIZM ENCODERS 1 or 2 =============================
...
void PRIZM5::resetEncoders(){					// ================== Reset BOTH PRIZM Encoders at once =========================
...
int PRIZM5::readMotorBusy (int channel){			// ================== Read Busy Status of PRIZM DC motor channel =======================
...
int PRIZM5::readMotorCurrent (int channel){		// ================ Read DC motor current of PRIZM DC channels ======================
...
void PRIZM5::setMotorInvert (int channel, int invert){						// ======================== Set the PRIZM DC Motor Direction invert status ====================
